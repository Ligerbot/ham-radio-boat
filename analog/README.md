...
`out = sstv.gray8(image)` which takes 8 seconds to tx but is grayscale. If you are decoding with the Robot36 app, just switch it to raw mode for this.

`out = sstv.Robot36(image)` 36 seconds, with color

`out = sstv.PD120(image)` 120 seconds roughly with color, higher resolution but takes more time.
